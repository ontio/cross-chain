...
当商户完成多签与合约的绑定后，商户就可以设置构造解锁BTC交易的参数了。首先需要商户确定下参数，然后各自用自己的比特币私钥对参数进行签名，填写参数签名即可。
...
| 私钥（WIF）       | WIF格式的比特币私钥；                                        |
| 多签Redeem        | 商家的多签赎回脚本；                                         |
| 参数版本          | 参数的版本号，参数可以更改，但是每次更改都只能增加版本号；   |
| 费率（sat/byte）  | 构造解锁交易的手续费率，即每字节多少聪，比如10sat/byte，那么一笔100字节的交易需要1000聪的手续费，这笔手续费从用户的解锁金额中扣除； |
| 最小找零值（sat） | 在构造解锁交易的时候，可能产生对多签地址的找零，最小找零值限制了找零的金额不能小于该值，防止产生过小的UTXO，甚至粉尘UTXO，伤害到跨链解锁的效果； |

### 5. 设置交易参数

当对参数的签名满足多签要求后，签名可以每个人发送自己的，也可以收集好签名一起发送，只要中继链上的签名足够多就会完成交易参数的设置，如下图，填入参数即可发送交易至中继链，完成参数设置。
